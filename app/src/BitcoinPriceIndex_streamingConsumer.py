...
def streamingPrice(config, producer_host, producer_port, master="local[2]", appName="CurrentPrice"):
    """
    ...
    dstream = strc.socketTextStream(producer_host, producer_port)
...
if __name__ == "__main__":
    ...
    streamingPrice(
            config,
            producer_host=config['price_streaming']['host'],
            producer_port=config['price_streaming']['port']
    )
